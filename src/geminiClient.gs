...
/**
 * 分析用システム指示(§5.2 プロンプトv4)。
 * 版数管理: v1(2026-07-10 初版) / v2(2026-07-14 画像マルチモーダル対応。ルール8・9を追加) /
 *   v3(2026-07-16 出力単位をメッセージごと→タスクごとに変更。ルール7を書き換え) /
 *   v4(2026-08-19 1メッセージに複数用件が含まれる場合の帰属先をルール7へ明記。
 *      同一msg_idが複数要素に列挙され形式検証で分析エラーになる事象への対策)。
...
const ANALYSIS_SYSTEM_PROMPT = [
  ...
  '   現れる必要があります。同じ msg_id を複数の要素に入れてはいけません。1つの',
  '   メッセージに複数の用件が含まれる場合は、主となる用件の要素1つだけに msg_id を',
  '   入れ、その要素の needsReview を true にしてください(人が残りの用件を確認します)。',
