...
const CONFIG = {
  ...
  // 日次サマリの「分析失敗◯件」の集計窓をタスクの対象窓から過去へずらす量(§4.4)。
  // エラー確定は受信から最大35分ほど遅れる(5分間隔×5リトライ+まとめ待機の持ち越し)ため、
  // タスクと同じ窓では「送信直前に受信→送信後にエラー確定」の行がどの窓にも入らず漏れる。
  // 窓全体を1時間ずらすことで各エラーがちょうど1つの窓に入る(漏れ・重複通知なし)
  SUMMARY_ERROR_WINDOW_LAG_MS: 60 * 60 * 1000,
